...
class ControllerDesignBanner extends Controller {
	...
	protected function getForm() {
		...
		foreach ($banner_images as &$banner_image_set) {
			foreach($banner_image_set as &$banner_image){
				if (!$banner_image['image'] || !file_exists(DIR_IMAGE . $banner_image['image'])) {
					$banner_image['image'] = 'no_image.jpg';
				}
				$banner_image['thumb'] = $this->model_tool_image->resize($banner_image['image'], 100, 100);
			}
		}
	...
	protected function validateForm() {
		...
		if (isset($this->request->post['banner_image'])) {
			foreach ($this->request->post['banner_image'] as $row_id => $banner_lang_group) {
				foreach ($banner_lang_group as $language_id => $banner_image) {
					if ((utf8_strlen($banner_image['title']) < 2) || (utf8_strlen($banner_image['title']) > 64)) {
						$this->error['banner_image'][$row_id][$language_id] = $this->language->get('error_title'); 
					}
				}
			}
		}
