...
class ControllerCatalogCategory extends Controller { 
	...
	protected function getList() {
		...
		$category_total = $this->model_catalog_category->getTotalCategoriesByParent(0);

		$results = $this->model_catalog_category->getCategoriesByParentId($data);
		if(!empty($results)) {
			foreach ($results as $result) {
				$hasChilds = $this->model_catalog_category->categoryHasChilds($result['category_id']);
				$action = array();
	...
	protected function getForm() {
		...
		$this->data['text_is_information'] = $this->language->get('text_is_information');
		...
		$this->data['entry_is_information'] = $this->language->get('entry_is_information');
		...
		if (isset($this->request->post['is_information'])) {
			$this->data['is_information'] = $this->request->post['is_information'];
		} elseif (!empty($category_info)) {
			$this->data['is_information'] = $category_info['is_information'];
		} else {
			$this->data['is_information'] = 0;
		}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$data = array(
				...
				'start'       	   => 0,
				'exclude_category' => (isset($this->request->get['exclude_category'])?$this->request->get['exclude_category']:null),
				'limit'       	   => 1000
			);
	...
	public function getCategoriesList() {
		if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest')
		{
			if(isset($this->request->post['parent_id'])) {
				$this->load->model('catalog/category');
				$categories = $this->model_catalog_category->getCategoriesByParentId(array('parent_id' => $this->request->post['parent_id']));
				foreach ($categories as &$category) {
					$category['href'] = $this->url->link('catalog/category/update', 'category_id=' . $category['category_id'] . '&token=' . $this->session->data['token'], 'NOSSL');
					$category['hasChilds'] = $this->model_catalog_category->categoryHasChilds($category['category_id']);
					$category['level'] = $this->model_catalog_category->getCategoryLevel($category['category_id']);
				}
			...
			die('This method is callable only by xhr ;-) ');
		}
	}
}
?>