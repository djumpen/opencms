...
class ControllerCommonFileManager extends Controller {
	...
	public function image() {
		...
		if (isset($this->request->get['image']) && !isset($this->request->get['fast-edit']) && !isset($this->request->get['json'])) {
			$this->response->setOutput($this->model_tool_image->resize(html_entity_decode($this->request->get['image'], ENT_QUOTES, 'UTF-8'), 100, 100));
		} elseif(isset($this->request->get['image']) && !isset($this->request->get['fast-edit']) && isset($this->request->get['json'])){
			$this->response->setOutput(json_encode(array(
					'thumb' => $this->model_tool_image->resize(html_entity_decode($this->request->get['image'], ENT_QUOTES, 'UTF-8'), 100, 100),
					'image' => $this->request->get['image'])));
		} elseif(isset($this->request->get['image']) && isset($this->request->get['fast-edit']) && isset($this->request->get['product_id'])) {
		    $this->load->model('catalog/product');
		    if($this->model_catalog_product->updateImage($this->request->get['product_id'], $this->request->get['image'])) {
    		    $this->response->setOutput(json_encode(array(
    	            'thumb' => $this->model_tool_image->resize(html_entity_decode($this->request->get['image'], ENT_QUOTES, 'UTF-8'), 40, 40),
                    'image' => $this->request->get['image'],
                )));
		    } else {
			...
			if (@move_uploaded_file($this->request->files['image']['tmp_name'], $directory . '/' . $filename)) {
				...
				if(file_exists(DIR_IMAGE . 'data/watermark/watermark.png')){
					...
					if ($info['width'] >= 200 && $info['height'] >= 200){
						$image->watermark(DIR_IMAGE . 'data/watermark/watermark.png');
