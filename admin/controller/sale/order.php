...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$this->data['orders'][] = array(
				...
				'comments_quantity' => $this->model_sale_order->getTotalOrderComments($result['order_id']),
				'action'        => $action,
				'products'		=> $tooltip_products
			);
		}
	...
	public function getForm() {
		...
		$this->data['show_group'] = (count($this->model_sale_customer_group->getCustomerGroups()) > 1);
		...
		foreach ($order_products as $order_product) {
			...
			if (isset($order_product['date_cancel'])) {
				$time_cancel = sprintf($this->language->get('text_cancel'),$order_product['date_cancel']);
				$date_cancel = $order_product['date_cancel'];
			} else {
				$time_cancel = '';
				$date_cancel = '';
			}
			...
			$this->data['order_products'][] = array(
				...
				'quantity'         => $order_product['quantity'] . $time_cancel,
				'time_cancel'	   => $date_cancel,
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['payment_city']) < 3) || (utf8_strlen($this->request->post['payment_city']) > 128)) {
			$this->request->post['payment_city'] = '   ';
		}
		...
		if ($country_info) {
			if ($country_info['postcode_required'] && (utf8_strlen($this->request->post['payment_postcode']) < 2) || (utf8_strlen($this->request->post['payment_postcode']) > 10)) {
				$this->request->post['payment_postcode'] = '';
			}
...
// 			$this->request->post['payment_country'] = $this->language->get('error_country');
// 		}

		if (!isset($this->request->post['payment_zone_id']) || $this->request->post['payment_zone_id'] == '') {
			$this->request->post['payment_zone_id'] = 1;
		}

		if ((!isset($this->request->post['payment_method']) || $this->request->post['payment_method'] == '') && $this->config->get('use_payment_methods')) {
...
// 				$this->error['shipping_postcode'] = $this->language->get('error_postcode');
				$this->request->post['shipping_postcode'] = '';
			}

			if ($this->request->post['shipping_country_id'] == '') {
// 				$this->error['shipping_country'] = $this->language->get('error_country');
				$this->request->post['shipping_country_id'] = 1;
			}

			if (!isset($this->request->post['shipping_zone_id']) || $this->request->post['shipping_zone_id'] == '') {
// 				$this->error['shipping_zone'] = $this->language->get('error_zone');
				$this->request->post['shipping_zone_id'] = 1;
			}

			if (!$this->request->post['shipping_method'] && $this->config->get('use_shipping_methods')) {
	...
	public function info() {
		...
		if ($order_info) {
			...
			$cart_options = $this->config->get('quickcheckout');
			
			// @todo: !!!We must use payment address as primary when we dont use different address for payment and shipping.
			$this->data['show_country'] = false;
			$this->data['show_postcode'] = false;
			$this->data['show_zone'] = false;
			if(isset($cart_options['option'])) {
				if(
					$cart_options['option']['guest']['payment_address']['display'] &&
					$cart_options['option']['guest']['payment_address']['fields']['country_id']['display']
				) {
					$this->data['show_country'] = true;
				}
				if(
					$cart_options['option']['guest']['payment_address']['display'] &&
					$cart_options['option']['guest']['payment_address']['fields']['postcode']['display']
				) {
					$this->data['show_postcode'] = true;
				}
				if(
					$cart_options['option']['guest']['payment_address']['display'] &&
					$cart_options['option']['guest']['payment_address']['fields']['zone_id']['display']
				) {
					$this->data['show_zone'] = true;
				}
			} else {
				$this->data['show_country'] = true;
				$this->data['show_postcode'] = true;
			...
			$this->data['date_added'] = date($this->language->get('datetime_format'), strtotime($order_info['date_added']));
			$this->data['date_modified'] = date($this->language->get('datetime_format'), strtotime($order_info['date_modified']));		
			$this->data['payment_firstname'] = $order_info['payment_firstname'];
			$this->data['payment_lastname'] = $order_info['payment_lastname'];
			$this->data['payment_company'] = $order_info['payment_company'];
			$this->data['payment_company_id'] = $order_info['payment_company_id'];
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				if ($order_info['payment_address_format']) {
					...
					$format = '{firstname} {lastname}' . "\n" . '{company}' . "\n" . '{address_1}' . "\n" . '{address_2}' . "\n" . '{city} {postcode}' . "\n";
					if($this->config->get('use_countries_and_regions')) {
						$format .= '{zone}' . "\n" . '{country}';
					}
				}
