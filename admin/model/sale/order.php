...
class ModelSaleOrder extends Model {
	...
	public function getTotalSalesByMonth($year, $month) {
		$sql = "
			...
			WHERE
				order_status_id > '0'
			AND
				YEAR(date_added) = '" . (int)$year . "'
			AND
				MONTH(date_added) = '" . (int)$month . "'
		";
		$query = $this->db->query($sql);
	...
	public function getTotalOrderComments($order_id) {
		$sql = "
			Select count(order_history_id) as quantity 
	...
	public function getEmailsOfCustomSubscribers() {
	    $sql = 'Select email From `' . DB_PREFIX . 'news_subscription`';
