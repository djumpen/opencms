...
class ModelPavblogblog extends Model { 
	...
	public function getBlogs() {
	    $sql = "
            Select * From " . DB_PREFIX . "pavblog_category as pc
            Left Join " . DB_PREFIX . "pavblog_category_description pbc On (pc.category_id = pbc.category_id)
            Where 1
            And pbc.language_id = " . $this->config->get('config_language_id') . "
        ";
	...
	public function getCategory($category_id) {
		$sql = "
			Select * 
			From 
				" . DB_PREFIX . "pavblog_category pvc
			Left Join 
				" . DB_PREFIX . "pavblog_category_description pvcd 
				On (pvc.category_id = pvcd.category_id)
			Where 
				pvcd.language_id = " . (int)$this->config->get('config_language_id') . " 
			And 
				pvc.category_id = " . (int)$category_id . "
		";
