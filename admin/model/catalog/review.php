...
class ModelCatalogReview extends Model {
	public function addReview($data) {
		$this->db->query("
			...
			SET 
				...
				purchased = '" . (isset($data['purchased'])?(int)$data['purchased']:0) . "', 
	...
	public function getReviews($data = array()) {
		$sql = "SELECT r.review_id, r.purchased, html_status, r.addimage, pd.name, r.author, r.rating, r.status, r.date_added FROM " . DB_PREFIX . "review r LEFT JOIN " . DB_PREFIX . "product_description pd ON (r.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' And (author <> '' AND author is not null)";																																					  
